...
RSpec.describe BibRecord do
  ...
  describe "#initialize" do
    it "reads json into marc field" do
      expect(bibrecord.marc).to be_a MARC::Record
    end
  end

  describe "#ht_bib_key" do
    it "extracts the bib key" do
      expect(bibrecord.ht_bib_key).to eq("011338539")
    end
  end

  describe "#oclc_num" do
    it "extracts the OCLC numbers" do
      expect(bibrecord.oclc_num).to eq(["5971627", "63970006"])
    end
  end

  describe "#isbn" do
    it "extracts the ISBN" do
      expect(bibrecord.isbn).to eq(["091072413X"])
    end
  end

  describe "#issn" do
    it "extracts the ISSN from the 022a" do
      expect(bibrecord.issn).to eq(["1474-0699", "1356-1898", "0041-977X"])
    end
  end

  describe "#lccn" do
    it "extracts the LCCN from the 010a" do
      expect(bibrecord.lccn).to eq(["70518371"])
    end
  end

  describe "#title" do
    it "extracts the title from the 245abcnp" do
      expect(bibrecord.title).to eq(["Committee on Foreign Relations, United States--subcommittees / February 1976."])
    end
  end

  describe "#imprint" do
    it "extracts the imprint from the 260bc" do
      expect(bibrecord.imprint).to eq(["U.S. Govt. Print. Off., 1976."])
    end

    it "extracts the imprint from the 264" do
      imp_rec = File.read(File.dirname(__FILE__) + "/data/imprint_rec.json")
      bibrecord = described_class.new(imp_rec)
      expect(bibrecord.imprint).to eq(["L'Officiel de la couture et de la mode de Paris."])
    end
  end
  ...
  describe "#u_and_f?" do
    ...
    it "detects presence of the u and f in the 008" do
      expect(bibrecord.u_and_f?).to be true
    end
  end

  describe "#pub_place" do
    it "has a POP from the 008" do
      expect(bibrecord.pub_place.to_s).to eq("dcu")
    end
  end

  describe "#author" do
    it "extracts the author from the 100abcd, 110abcd, 111acd" do
      expect(bibrecord.author).to eq(["United States. Congress. Senate. Committee on Foreign Relations"])
    end
  end

  describe "#lang" do
    it "extracts the language code" do
      expect(bibrecord.lang).to eq("eng")
    end
  end

  describe "#bib_fmt" do
    it "extracts the bib_fmt" do
      expect(bibrecord.bib_fmt).to eq("BK")
    end
  end

  describe "#sdr_nums" do
    it "assembles a mapping of collection codes to bib numbers" do
      expect(bibrecord.sdr_nums).to eq({"miu" => ["990058493500106381"],
    ...
    it "handles ILOC items appropriately" do
      ...
      bibrecord = described_class.new(m)
      expect(bibrecord.sdr_nums["iloc"]).to eq(["8200786"])
    end
  end

  describe "#item_records" do
    it "generates a list of ItemRecords from the 974s" do
      item_records = bibrecord.item_records.to_a
